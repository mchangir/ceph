...
class MDSRank {
  public:
    ...
    bool register_quarantine_mgr(inodeno_t ino, std::shared_ptr<QuarantineManager> qtine_mgr);
    void unregister_quarantine_mgr(inodeno_t ino);
    std::shared_ptr<QuarantineManager>& get_quarantine_mgr(inodeno_t ino);
...
private:
    ...
    std::mutex qtine_mutex;
    std::unordered_map<inodeno_t, std::shared_ptr<QuarantineManager>> qtine_mgrs; // subvol ino -> qtine mgr
};
