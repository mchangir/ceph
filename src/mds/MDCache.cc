...
class C_MDC_DataUninlinedSubmitted : public MDCacheLogContext {
  ...
  void finish(int r) {
    ...
    if (r) {
      ...
      in->mdcache->logger->inc(l_mdc_uninline_journaling_failed);
    } else {
...
void MDCache::uninline_data_work(MDRequestRef mdr)
{
  ...
  char oid_buf[32];
  snprintf(oid_buf, sizeof(oid_buf), "%llx.00000000", (long long unsigned)ino().val);
...
void MDCache::register_perfcounters()
{
    ...
    pcb.add_u64_counter(l_mdc_uninline_journaling_failed, "uninline_journaling_failed",
                        "Internal Counter type uninline journaling failed");
